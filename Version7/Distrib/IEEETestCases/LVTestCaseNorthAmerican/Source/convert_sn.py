...
def write_dss_model(fname, bParallel=False):
    op = open (fname, 'w')
    print ('clear', file=op)
    print ('Set DefaultBaseFrequency=60', file=op)
    ...
    if bParallel:
        print ("""
New Linecode.trans1 nphases=3  Units=mi
~ Rmatrix=[0.14519  |0.0925205  0.14519  |0.0925186  0.0925205  0.14519  ]
~ Xmatrix=[1.41011  |0.695545  1.41011  |0.611437  0.695545  1.41011  ]
~ Cmatrix=[12.4232  |-3.1131  12.989  |-1.63202  -3.1131  12.4232  ]

New Linecode.pri1 nphases=3  Units=mi
~ Rmatrix=[0.13018  |-4.14314E-006  0.130177  |-8.11898E-006  -4.14314E-006  0.13018  ]
~ Xmatrix=[0.0668195  |-0.000224891  0.066985  |-4.2835E-005  -0.000224891  0.0668195  ]
~ Cmatrix=[566.228  |0  566.228  |0  0  566.228  ]

New Linecode.sec1 nphases=3  Units=mi
~ Rmatrix=[0.33619  |0.128579  0.328163  |0.132719  0.128579  0.33619  ]
~ Xmatrix=[0.510307  |0.268686  0.575146  |0.193614  0.268686  0.510307  ]
~ Cmatrix=[34.0866  |-12.0789  34.1952  |-4.12143  -12.0789  34.0866  ]

New Linecode.sec5 nphases=3  Units=mi
~ Rmatrix=[0.067238 | 0.025716 0.065633 | 0.026544 0.025716 0.067238]
~ Xmatrix=[0.102061 | 0.053737 0.115029 | 0.038723 0.053737 0.102061]
~ Cmatrix=[170.4330 | -60.3945 170.9760 | -20.60715 -60.3945 170.4330]

New Linecode.sec6 nphases=3  Units=mi
~ Rmatrix=[0.056032 | 0.021430 0.054694 | 0.022120 0.021430 0.056032]
~ Xmatrix=[0.085051 | 0.044781 0.095858 | 0.032269 0.044781 0.085051]
~ Cmatrix=[204.5196 | -72.4734 205.1712 | -24.72858 -72.4734 204.5196]""", file=op)
    else:
        print (' ', file=op)
        ...
        if row[7] == 'Switch':
            ...
            if bParallel:
                print ('new Line.' + row[0] + ' bus1=' + row[1] + DSSPhases(row[2])
                   ...
                   + ' units=' + row[6] + ' linecode=' + row[7] + row[9], file=op)
            else:
                print ('new Line.' + row[0] + ' bus1=' + row[1] + DSSPhases(row[2])
                   + ' bus2=' + row[3] + DSSPhases(row[4]) + ' phases=3 length=' + row[5]
    ...
    print ('calcv', file=op)
    print ('buscoords buscoords.dat', file=op)
    print ('solve', file=op)
    op.close()


def write_atp_model(fname):
    # ATP output
    #print(seqParms)
    #print(waveParms)
    op = open (fname, 'w')
...
if __name__ == "__main__":
    write_dss_model ('SecNet.dss', bParallel=False)
    write_dss_model ('SecPar.dss', bParallel=True)
    # write_atp_model ('SecNet.atp')
